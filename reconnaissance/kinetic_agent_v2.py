...
# Fossilized parameters from the Eusocial-Combinatorial Heuristic Compiler (100 Gens)
...
def agent(obs):
    ...
    # Evaluate every planet we own for potential launches
    for source in my_planets:
        # GENOME: min_ship_threshold
        if source[5] < MIN_SHIP_THRESHOLD:
            continue
            
        best_target, best_score, best_angle, best_ships_to_send = None, -float('inf'), 0, 0
        
        for target in enemy_planets:
            tx, ty = target[2], target[3]
            target_prod = target[6]
            
            # Predict interception kinematics for our entire available fleet size to gauge speed
            max_ships_to_send = source[5] - 1 # Always leave 1 behind
            ...
            # GENOME: defense_buffer (Evolved to 0.0, we send exactly what is needed + 1 to conquer)
            ships_needed = math.ceil(target_defense_at_arrival + DEFENSE_BUFFER + 1)
            
            if source[5] > ships_needed:
                # We can afford it. Recalculate kinematics with exact ship count (slower speed)
                ...
                # Check defense one last time with exact slower travel time
                ...
                if source[5] > final_ships_needed:
                    ...
                    # High reward for high production targets, heavy penalty for travel time
